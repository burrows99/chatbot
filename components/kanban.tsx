...
export function KanbanBoardHiddenText({
  ...
  return (
    <div className={cn("hidden", className)} id={id} ref={ref} {...props}>
      {value}
    </div>
  );
}
...
export const KanbanBoardAccessibility = ({
  ...
  const markup = (
    <>
      ...
      />
      <KanbanBoardLiveRegion announcement={announcement} id={liveRegionId} />
    </>
  );
...
export function KanbanBoardCardTextarea({
  ...
  useImperativeHandle(
    externalReference as Ref<HTMLElement>,
    () => internalReference.current!
  );
